...
class DaoTest extends FunSuite with BeforeAndAfterEach {
  ...
  test("update backup set files") {
    ...
    assertResult(files)(dao.listFilesInBackupSet(bs.id).unwrappedFutureValue)
    ...
    assertResult(files2)(dao.listFilesInBackupSet(bs.id).unwrappedFutureValue)
  }
  ...
  test("listBackupSetsToExecute") {
    ...
    val List(bs, bs2, bs3, bs4) = dao.listAllBackupSets().unwrappedFutureValue
